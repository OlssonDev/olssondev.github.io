...
Now we have four arguments, instead of three. If you wan't to define a class/struct pin, you need to provide the class aswell for the **PinSubCategory** argument.

Here are two more examples of that:

```javascript
//Creates an Actor reference pin.
CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Object, AActor::StaticClass(), TEXT("My Actor Reference Pin"));

//Creates a struct pin with type GameplayTag.
CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Struct, FGameplayTag::StaticStruct(), "My GameplayTag Struct Pin");
``` 
