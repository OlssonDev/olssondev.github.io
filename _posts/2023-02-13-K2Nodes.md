...
You see, we now have four arguments instead of three when calling **CreatePin**. To declare a class/struct pin, you **must** provide the class for the **PinSubCategory** argument.

Here are two more examples of that:

```javascript
//Creates an Actor reference pin.
CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Object, AActor::StaticClass(), NAME_ActorReferencePin);
