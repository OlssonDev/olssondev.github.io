...
As you can see above we have two versions of making a ExecPin, and both are valid in the sense that they work, but Version 1 is a better alternative. **UEdGraphSchema_K2** is a great class to get names of different pin types, and should be used when choosing the pin's PinCategory.

You don't have to use it for the pin's name as I did in Version 1, if you need a custom name, go ahead and make one, but keep in mind that **GetExecPin()** will not work then.
