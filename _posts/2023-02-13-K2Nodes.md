...
In this blog post, we will make an IsValid node from scratch with a custom K2Node.

To create a K2Node from scratch, you need to override a few functions.
...
Your node is fully completed visually but can't do anything and generates Blueprint compiler errors. 

## Add logic & Blueprint compiler
...
Now to the the rest of the node:
...
To do that, we add this to the end of the ExpandNode function:
...
I'm going to keep adding stuff to this blog post as time passes, so stay tuned.

If you have any questions, just message me on Discord at **Olsson#8391**